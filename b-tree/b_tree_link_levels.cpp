...
//R is right pointer which is used when adding median
void insert (BDPTR &T, int k) {
	T->key[T->n] = k;
	++T->n;
	int i;
...
void splitNode (BDPTR &oldNode, BDPTR &P) {
	...
	newNode->rptr = oldNode->rptr;
	oldNode->rptr = newNode;
...
//Add key to B-Tree
//P = Parent, R = Right pointer used for adding median, L = Left pointer used for adding median
void addNode (BDPTR &T, int k, BDPTR P) {
	if(T) {
		int i;
		//If leaf node
...
//Print using level pointers
void printLvlPtr (BDPTR T) {
	if(T) {
		BDPTR I = T;
		int i;
		while(I) {
