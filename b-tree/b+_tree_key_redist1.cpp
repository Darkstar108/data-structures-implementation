...
//Check if there exists adjecent sibling with space
int checkSibling (BDPTR P, BDPTR T, int i = 0) {
	if(i == P->n+1)
		return -1;
	if(P->ptr[i] == T) {
		return checkSibling(P, T, i+1);
	}
...
//Perform B+ Node Splitting
void splitNodeLeaf (BDPTR &oldNode, BDPTR &P) {
	BDPTR newNode = new(bdnode);
	int i;
	//Copying right of median to new node
	copy(newNode, oldNode, 0, d+1, d);
	newNode->ptr[0] = oldNode->ptr[d];
	newNode->n = d+1;
	//Making rest of new node empty
	copy(newNode, NULL, d+1, 2*d+1);
	//Store median
	i = oldNode->key[d];
...
//Key Redistribution 1 (With adjecent siblings)
void keyRedist (BDPTR &T, BDPTR &P, int s) {
	int t;
	for(t = 0; t <= 2+d; ++t) {
		if(P->ptr[t] == T)
	...
	if(T) {
		...
		if(T->n == 2*d+1) {
			...
			if(P == NULL)
				...
				//If no adjacent sibling with space
				if(s == -1)
					if(isLeaf(T) == 1)
						splitNodeLeaf(T, P);
					else
...
{
	BDPTR T = NULL;
	//int a[] = {39,34,22,27,2,62,60,75,40,50}, n = 10, i;
